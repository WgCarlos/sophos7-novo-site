<?php 
	include "inc/header.php";
	include "inc/menu.php";
?>
<div class="tituloPagina">
	<div class="container">
		<div class="row">
			<div class="col-sm-6">
				<div>
					<h1>Versão Completa</h1>
				</div>
			</div>
		</div>
	</div>
</div>
<div class="imgService text-center">
	<img src="img/UNIPLUSVertical.png" class="img-fluid">
</div>
<div class="container">
	<div class="serviceSolucao">
		<div class="row">
			<div class="offset-sm-2 col-sm-8">
				<h3 class="font-weight-bold text-center">Solução completa para seu negócio</h3>
				<p class="text-center">Administrar uma empresa não é uma tarefa fácil, são muitos os detalhes com os quais se preocupar. Pensando nisso, desenvolvemos o UNIPLUS, um sistema de gestão que se adapta às necessidades da sua empresa. Gestão simples que facilita os processos colocando em suas mãos o total controle do seu negócio.</p>
			</div>	
		</div>
	</div>
</div>
<div class="imgUniplus">
	<div class="container">
		<div class="embed-responsive embed-responsive-4by3">
  			<iframe src="https://www.youtube.com/embed/bEq7uayZW-g" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
		</div>
	</div>
</div>
<div class="container-fluid">
	<div class="serviceSolucao1">
		<div class="headerService">
			<h3 class="text-center font-weight-bold">Nós temos o que você precisa</h3>
		</div>
		<div class="row">
    		<div class="col">
    			<div class="text-center">
      				<img src="img/uniplusnotaFiscal.png">
      				<p class="">Nota Fiscal</p>
      			</div>
    		</div>
    		<div class="col">
      			<div class="text-center">
      				<img src="img/controle.png">
      				<p class="">Controle Financeiro</p>
      			</div>
    		</div>
    		<div class="col">
      			<div class="text-center">
      				<img src="img/pedido.png">
      				<p class="">Pedido de Faturamento</p>
      			</div>
    		</div>
    		<div class="col">
      			<div class="text-center">
      				<img src="img/grade1.png">
      				<p class="">Grade</p>
      			</div>
    		</div>
    		<div class="col">
      			<div class="text-center">
      				<img src="img/fichaproducao.png">
      				<p class="">Ficha de  Produção</p>
      			</div>
    		</div>
    		<div class="col">
      			<div class="text-center">
      				<img src="img/os1.png">
      				<p class="">Ordem de Serviço</p>
      			</div>
    		</div>
  		</div>
	</div>
	<div class="headerService1">
		<div class="row">
			<div class="offset-sm-3 col-sm-6">
				<p class="text-center ">O UNIPLUS é um sistema de gestão completo, rápido, prático e sem complicações. Com o UNIPLUS a empresa evolui de maneira saudável e bem estruturada, contando com módulos adicionais focados em seu segmento.</p>
			</div>	
		</div>	
	</div>
</div>
<div class="container">
	<div class="serviceSolucao2">
		<h3 class="text-center font-weight-bold">Versões Disponíveis</h3>
	</div>
	<div class="row">
		<div class="col-sm-4">
			<div class="card" style="width: 20rem;">
		  		<img class="card-img-top" src="img/SEHorizontal.png" alt="Card image cap">
		  		<div class="card-body">
		  			<ul class="list-group list-group-flush">
		    		<h5 class="text-center card-title font-weight-bold">Básico</h5>
			    		<p class="text-center">Até 03 usuários</p>
			    		<p class="card-text"><strong>Funcionalidades</strong></p>
			    		<li class="list-group-item"><i class="fas fa-check"> Cadastros</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Estoque</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Financeiro</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Modelos de Impressão</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Nota Fiscal Eletrônica</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Produtos</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Relatórios</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> SINTEGRA</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> SPED Fiscal</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Vendas</i></li>
			    		<a href="contato.php" class="btn btn-primary">Tenho Interesse</a>
		    		</ul>
		  		</div>
			</div>
		</div>
		<div class="col-sm-4">
			<div class="card" style="width: 20rem;">
		  		<img class="card-img-top" src="img/uniplus.png" alt="Card image cap">
		  		<div class="card-body">
		  			<ul class="list-group list-group-flush">
		    		<h5 class="text-center card-title font-weight-bold">Avançado</h5>
			    		<p class="text-center">03 usuários ou +</p>
			    		<p class="card-text"><strong>Funcionalidades</strong></p>
			    		<li class="list-group-item"><i class="fas fa-check"> Cadastros</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Estoque</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Financeiro</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Modelos de Impressão</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Nota Fiscal Eletrônica</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Produtos</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Relatórios</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> SINTEGRA</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> SPED Fiscal</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Vendas</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> EDI - Coletor de Dados</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Estoque de Terceiros</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Ficha Técnica de Produto</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Grade</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Ordem de Serviço</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Pedido de Compras</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Registro de Produção</i></li>
			    		<a href="contato.php" class="btn btn-primary">Tenho Interesse</a>
		    		</ul>
		  		</div>
			</div>
		</div>	
		<div class="col-sm-4">
			<div class="card" style="width: 20rem;">
		  		<img class="card-img-top" src="img/uniplus.png" alt="Card image cap">
		  		<div class="card-body">
		  			<ul class="list-group list-group-flush">
		    		<h5 class="text-center card-title font-weight-bold">Módulos Adicionais</h5>
			    		<p class="text-center">03 usuários ou +</p>
			    		<p class="card-text"><strong>Funcionalidades</strong></p>
			    		<li class="list-group-item"><i class="fas fa-check"> Atacarejo</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Ativo Imobilizado</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Boleto com Registro</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Cartão Fidelidade</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Nota Fiscal Eletrônica</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Centro de Custo</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Conciliador de Cartões</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Conferência de Mercadorias</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Conhecimento de Transporte (CT-e)</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Controle de Entrada de Mercadoria</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Controle de Entrega (Expedição)</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Controle de Lote</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Cotação de Compra</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Cotação Web</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Dashboard de Gestão</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> E-commerce (Loja Virtual)</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Enterprise ou Multiempresa</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Envio de SMS</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Família de Produtos</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Gestão de Contratos</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Guia Nacional de Recolhimento (GNRE)</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Help Desk (Gestão de Relacionamentos)</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Higienizador Fiscal (Mix Fiscal)</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Integração com Sistemas Contábeis</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Integração com TEF</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Inventário de Estoque</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Limite por Forma de Pagamento</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Manifesto de Carga (MDF-e)</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Nota Fiscal de Serviço (NFS-e)</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> PDV Mobile</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Programação de Alteração de Preço</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Promoção por Quota</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Reclassificação (Peças de Carnes)</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> SPED Contribuições (PIS/COFINS)</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Tabela de Preço</i></li>
			    		<li class="list-group-item"><i class="fas fa-check"> Vendas Mobile (Força de Vendas)</i></li>
			    		<a href="contato.php" class="btn btn-primary">Tenho Interesse</a>
		    		</ul>
		  		</div>
			</div>
		</div>		
	</div>
</div>
<?php 
	include "inc/footer.php";
?>